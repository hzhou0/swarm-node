...
from .gst_utils import (
    gst_state_to_str,
    fraction_to_str,
    gst_buffer_with_caps_to_ndarray,
)
...
class GstVideoSource(GstPipeline):
    ...
    def __init__(self, command: str, leaky: bool = False, max_buffers_size: int = 100):
        ...
        self._sink: GstApp.AppSink | None = None
        self._counter: int = 0  # counts number of received buffers
    ...
    def _on_pipeline_init(self):
        ...
        appsinks: list[GstApp.AppSink] = self.get_by_cls(GstApp.AppSink)
        self._sink: GstApp.AppSink = appsinks[0] if len(appsinks) == 1 else None
        if not self._sink:
