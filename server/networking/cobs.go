...
func (e CobsEncoder) Encode(src []byte) []byte {
	loopLen := len(src)
...
func (e CobsEncoder) Decode(src []byte) []byte {
	loopLen := len(src)
...
func (e CobsEncoder) Verify(src []byte) error {
	nextFlag := 0
	loopLen := len(src)
	if e.Delimiter {
		if loopLen < 2 {
...
func (e CobsEncoder) FlagCount(src []byte) int {
	ret := 0
	ptr := 0
	for ptr < len(src) {
		if src[ptr] == 0 {
...
func (e CobsEncoder) MaxOverhead(len int) int {
	ret := len + 1 + (len / 254)
	if e.Delimiter {
		ret++
	}
...
func (e CobsEncoder) MinOverhead(len int) int {
	ret := len + 1
	if e.Delimiter {
		ret++
	}
...
var cobsEncoder = CobsEncoder{0x0, false, false}
