...
class SwarmNodeClient:
    ...
    async def webrtc_offer(self, webrtc_offer: WebrtcOffer) -> WebrtcOffer:
        async with AsyncClient() as cl:
            r=await cl.put(self.root_url+"/api/webrtc", content=self.encoder.encode(webrtc_offer).decode("utf-8"))
        return self.offer_decoder.decode(r.raise_for_status().content)

    async def get_kernel_id(self)->str:
        async with AsyncClient() as cl:
            r=await cl.get(self.root_url+"/api/k")
